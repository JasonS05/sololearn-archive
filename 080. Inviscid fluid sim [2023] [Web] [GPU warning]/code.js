...
const simulationResolution = 512;
const targetFPS = 20;
const maxIterationsPerFrame = 1/0; // no speed limit

const texSize = simulationResolution * 3;

function main() {
	// get and initialize canvas
	const canvas = document.getElementById("can");
	canvas.width = canvas.height = simulationResolution;
	...
	let speedVorticityUniform = gl.getUniformLocation(displayShader, "speed_vorticity");
	gl.uniform1i(speedVorticityUniform, confirm("Show fluid vorticity instead of fluid speed? (in vorticity view, red means clockwise and blue means counterclockwise)")? 1 : 0);
	...
	function loop() {
		...
		if (!window.stopSim) requestAnimationFrame(loop);
	}
	
	window.restartSim = function() {
		window.stopSim = false;
