...
function main() {
	...
	let randomUniform = gl.getUniformLocation(initShader, "random");
	gl.uniform1f(randomUniform, Math.random());
	...
	function loop() {
		...
		if (averageFPS > targetFPS) {
			iterationsPerFrame *= 1 + Math.min(1, (averageFPS - targetFPS) / 10) / 100;
		} else if (averageFPS < targetFPS) {
			iterationsPerFrame *= 1 - Math.min(1, (targetFPS - averageFPS) / 10) / 100;
		}
		
		iterationsPerFrame = Math.min(Math.max(iterationsPerFrame, 1), maxIterationsPerFrame);
		...
		for (let i = 0; scheduledIterations > 0; i++, scheduledIterations--) {
			...
			let randomUniform = gl.getUniformLocation(compShader, "random");
			gl.uniform1f(randomUniform, Math.random());
