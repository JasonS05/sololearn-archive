...
struct miscData {
    timeStep: f32,
    backupTimer: u32,
    simBroken: u32 // stand-in for bool
}
...
@group(0) @binding(3) var<storage, read> input_misc: miscData;
...
@compute @workgroup_size(64) fn computeShader(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let timeStep = input_misc.timeStep;

    if (input_misc.simBroken == 0) {
        ...
        if (input_misc.backupTimer == 0) {
            output_misc.backupTimer = 100;

            output_data[index + numParticles] = particle;
            output_data[index + numParticles * 2] = input_data[index + numParticles];
        } else {
            output_misc.backupTimer = input_misc.backupTimer - 1;

            output_data[index + numParticles] = input_data[index + numParticles];
            output_data[index + numParticles * 2] = input_data[index + numParticles * 2];
        }

        output_misc.timeStep = timeStep * 1.0001;

        if (length(particle.velocity) * timeStep > particleRadius / 2) {
            output_misc.simBroken = 1;
        }
    } else {
        output_data[index + numParticles] = input_data[index + numParticles];

        let backupParticle = input_data[index + numParticles * 2];
        output_data[index + numParticles * 2] = backupParticle;

        output_data[index] = backupParticle;
        placeParticleInGrid(backupParticle);

        output_misc.timeStep = timeStep * 0.1;
        output_misc.backupTimer = 100;
        output_misc.simBroken = 0;
    }
}
...
async function main() {
    ...
    let workBuffer = new Float32Array(numParticles * 3 * bytesPerParticle);
